...
class CTC(nn.Module):
    ...
    def __init__(self, embed_dim, dictionary_size, dropout,blank_id,
    ...
    def valid(self, logits_or_probs, targets, input_lengths, dictionary):
        ...
        with torch.no_grad():
            ...
            for lp, t, inp_l in zip(
                ...
                pred_words_raw = post_process(pred_units, self.post_process).split()
                if(index == 0 ):
                    logger.info("gloss hypothesis: " + post_process(pred_units, self.post_process))
                    logger.info("gloss reference: " + post_process(targ_units, self.post_process))
                    index = 1
    ...
    def gen_gloss(self, logits_or_probs, lengths, targets=None):
        gloss_fake = []
        gloss_fake_length = []
        for lp, inp_l in zip(
                logits_or_probs,
            ...
            #pred_units_arr = toks
            gloss_fake.append(pred_units_arr)
            gloss_fake_length.append(len(pred_units_arr))
        return gloss_fake, gloss_fake_length