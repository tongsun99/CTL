...
@register_criterion(
    "label_smoothed_cross_entropy_g2t", dataclass=LabelSmoothedCrossEntropyCriterionConfig
)
class LabelSmoothedCrossEntropyCriterionGlossToText(FairseqCriterion):
    def __init__(
            self,
            task,
            sentence_avg,
    ...
    def forward(self, model, sample, reduce=True):
        ...
        """
        if "gloss" in sample:
            sample["net_input"]["src_tokens"] = sample["gloss"]["tokens"]
            sample["net_input"]["src_lengths"] = sample["gloss"]["lengths"]
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        if "mixup" in net_output[1] and net_output[1]["mixup"] is not None:
            ...
            if self.mixup_consistent_weight > 0:
                ...
                for item_mixup_lprobs, tgt_lprobs, pad, coef in zip(mixup_lprobs, teacher_lprobs, target_pad_mask,
                                                                    loss_coef):
                    item_loss = F.kl_div(
