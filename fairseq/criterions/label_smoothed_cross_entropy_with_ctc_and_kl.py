...
class KL_LabelSmoothedCrossEntropyCriterionConfig(FairseqDataclass):
    ...
    gloss_translation_weight: float = field(
        default=0.0,
        metadata={"help": "weight of gloss translation loss"},
    )
    gloss_decoder_detach: bool = field(
        default=False,
        metadata={"help": "gloss_decoder_detach"},
    )
    cal_kl_mixup_loss: bool = field(
        default=False,
        metadata={"help": "calculate the kl loss for the mixed samples"},
    )
    cal_all_fake_loss: bool = field(
        default=False,
        metadata={"help": "calculate the fake loss"},
    )
    distance_loss_weight: float = field(
        default=0.0,
        metadata={"help": "calculate the distance loss for sign and gloss"},
    )



@register_criterion(
    "label_smoothed_cross_entropy_with_ctc_and_kl",
    dataclass=KL_LabelSmoothedCrossEntropyCriterionConfig,)
class LabelSmoothedCrossEntropyCriterionWithCTC(
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if self.ctc_criterion.all_ctc_weight > 0:
            ctc_loss, logging_output = self.ctc_criterion.compute_ctc_loss(model, sample, sign_encoder_out, logging_output)
            loss = loss + self.ctc_weight * ctc_loss
            logging_output["ctc_loss"] = utils.item(ctc_loss.data) if reduce else ctc_loss.data

        if self.kl_weight:
            self.temperature = 1.0
            kl_loss = self._get_ctc_self_distill_loss(model, gloss_decoder_output, net_output, sample,
                                                      self.temperature)
            loss = loss + self.kl_weight * kl_loss
            if self.cal_all_fake_loss:
                fake_kl_loss = self._get_ctc_self_distill_loss(model, gloss_decoder_output, fake_net_output, sample,
                                                               self.temperature)
    ...
    def _get_ctc_self_distill_loss(self,model,gloss_decoder_output, net_output, sample, temperature=1.0):
