...
class MUAgent(SignAgent):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--mu-model-path", type=str, required=True, 
                            help="Path to mu sign cls model")
        parser.add_argument("--mu-threshold", type=float, required=True,
                           help="mu threshold")
    ...
    def load_model_vocab(self, args):
        ...
        # mu model
        if not os.path.exists(args.mu_model_path):
            raise IOError("MU Model file not found: {}".format(args.mu_model_path))
        mu_state = checkpoint_utils.load_checkpoint_to_cpu(args.mu_model_path)
        mu_task_args = mu_state["cfg"]["task"]
        mu_task = tasks.setup_task(mu_task_args)
        self.mu_model = mu_task.build_model(mu_state["cfg"]["model"])
        self.mu_model.load_state_dict(mu_state["model"], strict=True)
        self.mu_model.eval()
    ...
    def update_model_encoder(self, states):
        ...
        with torch.no_grad():
            threshold = self.mu_model(source=src_indices, padding_mask=None)
        states.threshold = torch.nn.functional.softmax(threshold["pooled"], dim=-1)[0, 1].item()
    ...
    def policy(self, states):
        ...
        if states.threshold < self.args.mu_threshold:
            return READ_ACTION
