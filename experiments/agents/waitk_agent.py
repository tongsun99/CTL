...
class WaitkAgent(SignAgent):
    ...
    def add_args(parser):
        ...
        # waitk
        parser.add_argument("--waitk", type=int, default=1,
                            help="Wait k segments before decoding")
    ...
    def policy(self, states):
        ...
        states.incremental_states["steps"] = {
            "src": states.units.source.__len__(),
        ...
        if (not states.finish_read()
                and states.incremental_states["steps"]["src"] - len(states.units.target) * self.args.pre_decision_ratio < self.args.waitk * self.args.pre_decision_ratio
                ):
            return READ_ACTION
        else:
