...
class CTLDataAgent(SignAgent):
    ...
    def __init__(self, args):
        ...
        # read offline transl result
        self.instance_log = []
        with open(args.instance_log, "r") as f:
            lines = f.readlines()
        for line in lines:
            json_obj = json.loads(line) # {"index": 0, "prediction": ...}
            self.instance_log.append(json_obj)
        # remove ctl data if exist
    ...
    def build_states(self, args, client, sentence_id):
        ...
        states.ref = self.instance_log[sentence_id]["reference"] 
        states.hypo = self.instance_log[sentence_id]["prediction"].replace("</s>", "")
        hypo_indices = self.model.decoder.dictionary.encode_line(self.bpe.encode(states.hypo), append_eos=False, add_if_not_exist=False).long().unsqueeze(0)
        states.hypo_indices = torch.cat([torch.LongTensor([self.model.decoder.dictionary.eos()]).unsqueeze(0), hypo_indices], dim=-1)
        states.prev_cur_str = ""
        states.instance_id = sentence_id
    ...
    def add_args(parser):
        ...
        parser.add_argument("--instance-log", type=str, required=True,
                            help="Path of offline transl result")
        parser.add_argument("--output-file", type=str, required=True,
                            help="Path of ctl data")
    ...
    def policy(self, states):
        ...
        while True:
            # early stop
            if cur_indices.size(1) >= states.hypo_indices.size(1):
                break
        ...
        # translation before longest common prefix
        cur_str_ori = self.model.decoder.dictionary.string(cur_indices, bpe_symbol="sentencepiece")
        before_remove_len = cur_indices.size(1)

        # obtain the longest common prefix
        lcp = longest_common_prefix(states.hypo_indices, cur_indices)
        translation = self.model.decoder.dictionary.string(cur_indices[0][:lcp], bpe_symbol="sentencepiece")

        if not states.finish_read():
            cur_indices_len = cur_indices.size(1)
            if lcp - tgt_indices.size(1) > 0:   # new correct translation tokens
                after_remove_len = lcp
            else:
                self.write_output_file(states,
                                       translation_history=tgt_str,
                                       translation_ori=cur_str_ori,
                                       translation=translation,
                                       label=lcp - 1,
                                       tgt_indices_len=tgt_indices.size(1),
                                       before_remove_len=before_remove_len,
                                       label_rw=0)
        ...
        # actually, prefix matching is not needed
        if states.hypo.startswith(cur_str):
            # prevent infinite WRITE loops
            if not states.finish_read() and cur_str == states.prev_cur_str:
                self.write_output_file(states,
                                       translation_history=tgt_str,
    ...
    def write_output_file(self, states, **kwargs):
        with open(self.args.output_file, "a+") as f:
            json_obj = {
                "instance_id": states.instance_id,
                "nfeatures": states.units.source.__len__(),
            }
